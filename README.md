...
- **Appearance.** Each group's pane background, status-bar dot accent, and optional sound live in `~/.tmux.conf` as `@notify_<group>_bg|accent|sound` (plus `@notify_volume`). Edit there and reload with `tmux source-file ~/.tmux.conf`. `~/.tmux/conf.d/notify.conf` renders the status-bar flag and clears it when you return focus to the pane.
