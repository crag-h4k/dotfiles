    ...
    case "$os" in
        ...
        debian)
            ...
            if [[ "$INSTALL_ZSH" == true ]]; then
                ...
                # dns-sniff (~/.zsh/bin/dns-sniff) deps: tcpdump is its capture
                # engine; python3 is its interpreter. Both are built-in on macOS
                # but not installed by default on Debian.
