...
render_components() {
    ...
    local zsh tmux neovim gitconfig codecompanion claudesquad
    zsh=$(printf '%s\n' "$out" | sed -n 's/^[[:space:]]*zsh = \(.*\)$/\1/p')
    tmux=$(printf '%s\n' "$out" | sed -n 's/^[[:space:]]*tmux = \(.*\)$/\1/p')
    neovim=$(printf '%s\n' "$out" | sed -n 's/^[[:space:]]*neovim = \(.*\)$/\1/p')
    gitconfig=$(printf '%s\n' "$out" | sed -n 's/^[[:space:]]*gitconfig = \(.*\)$/\1/p')
    codecompanion=$(printf '%s\n' "$out" | sed -n 's/^[[:space:]]*codecompanion = \(.*\)$/\1/p')
    claudesquad=$(printf '%s\n' "$out" | sed -n 's/^[[:space:]]*claudesquad = \(.*\)$/\1/p')
    printf '%s %s %s %s %s %s' "$zsh" "$tmux" "$neovim" "$gitconfig" "$codecompanion" "$claudesquad"
}
...
# Args: selection expected_zsh expected_tmux expected_neovim expected_gitconfig expected_codecompanion expected_claudesquad
assert_case() {
    local selection="$1" want="$2 $3 $4 $5 $6 $7" got
    ...
    if [[ "$got" != "$want" ]]; then
        ...
        echo "  expected (zsh tmux neovim gitconfig codecompanion claudesquad): $want" >&2
        echo "  got     (zsh tmux neovim gitconfig codecompanion claudesquad): $got" >&2
        fail=1
    fi
}

# --- .chezmoi.toml.tmpl: exhaustive numeric matrix -------------------------
# All 64 on/off combinations of digits 1..6, expressed with NO spaces (e.g.
...
ncases=0
for d1 in 0 1; do for d2 in 0 1; do for d3 in 0 1; do for d4 in 0 1; do for d5 in 0 1; do for d6 in 0 1; do
    sel=""
    (( d1 )) && sel+="1"
    (( d2 )) && sel+="2"
    (( d3 )) && sel+="3"
    ...
    assert_case "$sel" \
        ...
        "$(has_digit 5 "$sel")" "$(has_digit 6 "$sel")"
    ncases=$((ncases + 1))
done; done; done; done; done; done

# --- keyword forms ---------------------------------------------------------
# all  = default-on set (zsh tmux neovim); gitconfig + codecompanion + claudesquad off.
assert_case "all"  true true true false false false
# all+ = everything including gitconfig, codecompanion and claudesquad.
assert_case "all+" true true true true true true
...
# not 3); "1,3" proves comma separators are ignored too. "3 5" proves
# codecompanion (digit 5) parses alongside neovim; "2 6" proves claudesquad
# (digit 6) parses alongside tmux.
assert_case "3 1"   true  false true  false false false
assert_case "421"   true  true  false true  false false
assert_case "1,3"   true  false true  false false false
assert_case "1 2 3" true  true  true  false false false
assert_case "3 5"   false false true  false true  false
assert_case "2 6"   false true  false false false true
...
assert_case ""                       true true true false false false
assert_case "Components to install:" true true true false false false
