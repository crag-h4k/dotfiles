...
render_components() {
    ...
    local zsh tmux neovim gitconfig codecompanion
    ...
    printf '%s %s %s %s %s' "$zsh" "$tmux" "$neovim" "$gitconfig" "$codecompanion"
}
...
# Args: selection expected_zsh expected_tmux expected_neovim expected_gitconfig expected_codecompanion
assert_case() {
    ...
    if [[ "$got" != "$want" ]]; then
        ...
        echo "  expected (zsh tmux neovim gitconfig codecompanion): $want" >&2
        echo "  got     (zsh tmux neovim gitconfig codecompanion): $got" >&2
        fail=1
    fi
}
...
# --- keyword forms ---------------------------------------------------------
# all  = default-on set (zsh tmux neovim), gitconfig + codecompanion off.
...
# codecompanion (digit 5) parses alongside neovim.
